...
pub struct Bus {
    topics: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, (broadcast::Sender<Message>, usize)>>,
    >,
    actors: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, ActorInfo>>,
    >,
    topic_subscribers: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, std::collections::HashSet<String>>>,
    >,
    topic_publishers: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, std::collections::HashSet<String>>>,
    >,
    announced_actors: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, ActorAnnouncement>>,
    >,
}
...
impl Bus {
    pub fn new() -> Self {
        Self {
            topics: std::sync::Arc::new(tokio::sync::RwLock::new(
                std::collections::HashMap::new(),
            )),
            actors: std::sync::Arc::new(tokio::sync::RwLock::new(
                std::collections::HashMap::new(),
            )),
            topic_subscribers: std::sync::Arc::new(tokio::sync::RwLock::new(
                std::collections::HashMap::new(),
            )),
            topic_publishers: std::sync::Arc::new(tokio::sync::RwLock::new(
                std::collections::HashMap::new(),
            )),
            announced_actors: std::sync::Arc::new(tokio::sync::RwLock::new(
                std::collections::HashMap::new(),
            )),
        }
    }
    ...
    pub async fn subscribe(
        ...
        actor_id: &str,
        topic: &str,
    ) -> Result<broadcast::Receiver<Message>, BusError> {
        let capacity = 1000;
        self.ensure_topic(topic, capacity).await;
    ...
    pub async fn list_announced_topics(&self) -> Vec<TopicAnnouncement> {
        let actors = self.announced_actors.read().await;
        let mut topics: std::collections::HashMap<String, TopicAnnouncement> =
            std::collections::HashMap::new();
        ...
        for announcement in actors.values() {
            for sub in &announcement.subscriptions {
                topics
                    .entry(sub.clone())
                    .or_insert_with(|| TopicAnnouncement {
                        name: sub.clone(),
                        subscribers: Vec::new(),
                        publishers: Vec::new(),
                    })
                    .subscribers
                    .push(announcement.id.clone());
            }
            for pub_topic in &announcement.publications {
                topics
                    .entry(pub_topic.clone())
                    .or_insert_with(|| TopicAnnouncement {
    ...
    pub async fn spawn_actor<A: Actor + Send + 'static>(
        ...
        info!(
            "Spawning actor: {} (announced {} subs, {} pubs)",
            actor_id,
            announcement.subscriptions.len(),
            announcement.publications.len()
        );
        self.register_announcement(announcement.clone())
            .await;
...
pub enum BusError {
    ...
    ChannelClosed,
    #[error("Actor '{0}' has not announced and cannot publish")]
